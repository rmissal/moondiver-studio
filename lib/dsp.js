...
function buildFilterChain(options = {}, measured = null, duration = null) {
  ...
  // 1. Auto-Lead Silence Trim (Beseitigt Vorlauf-Stille >500ms für 100% Apple Music Ingestion-Konformität, schont aber sanfte Intros)
  if (autoTrim) {
  ...
  // 4. Dynamisches De-Essing (Glättet schneidende Höhen & schrille Spitzen sanft und transparent)
  filters.push(`deesser=i=0.25:f=0.6:m=0.3`);

  // 5. Bass Warmth (Grundton-Wärme für akustische Instrumente & Pads)
  if (bassGainDb !== 0) {
    filters.push(`bass=g=${bassGainDb}:f=${base.bassFreq || 80}:w=0.6`);
  }

  // 6. Midrange Clarity (Sanfte Entmumpfung der unteren Mitten)
  if (midDeMudGainDb !== 0) {
    filters.push(`equalizer=f=${base.midDeMudFreq || 320}:t=q:w=1.0:g=${midDeMudGainDb}`);
  }

  // 7. Silky Treble Air (Seidiger Glanz ohne Härte oder Klirren)
  if (airTrebleGainDb !== 0) {
    filters.push(`treble=g=${airTrebleGainDb}:f=${base.airTrebleFreq || 11000}:w=0.5`);
  }

  // 8. Analoge Röhren-/Bandsättigung (Verleiht digitale Wärme & musikalischen Zusammenhalt)
  filters.push(`asoftclip=type=sin:threshold=0.98`);

  // 9. Räumliche Stereobreite (Sphärischer Raumklang)
  if (stereoWidth !== 1.0) {
