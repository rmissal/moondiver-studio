...
function buildFilterChain(options = {}, measured = null, duration = null) {
  ...
  if (stereoWidth && stereoWidth !== 1.0 && stereoWidth <= 1.10) {
    filters.push(`stereotools=slev=${stereoWidth}:mlev=1.0`);
  }

  // 8. EBU R128 Loudness Normalization & True Peak Limiter (Two-Pass Calibrated)
  ...
  if (autoFadeOut && measured) {
    ...
    filters.push(`areverse, silenceremove=start_periods=1:start_duration=0.2:start_threshold=-75dB, afade=t=in:ss=0:d=${fadeDur}:curve=exp, areverse`);
  }
  ...
  return {
    ...
    settings: {
      ...
      enhancedChain: ['auto_lead_silence_trim', 'anticlick_fadein', 'transparent_eq', 'ebur128_two_pass_linear', 'tail_trim_and_fadeout']
    }
  };
}
