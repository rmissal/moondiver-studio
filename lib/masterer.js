...
async function masterSingleFile(inputFile, options = {}) {
  ...
  const genreStr = options.genre || (isAuto && genreInfo ? genreInfo.genreName : (PRESETS[effectivePresetKey]?.name || effectivePresetKey));
  ...
  const result = {
    ...
    autoDetectedGenre: isAuto ? genreInfo : { preset: effectivePresetKey, name: PRESETS[effectivePresetKey]?.name || effectivePresetKey },
