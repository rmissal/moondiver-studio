...
async function masterSingleFile(inputFile, options = {}) {
  ...
  const baseName = rawBaseName.replace(/_Mixed$/i, '').replace(/_Master$/i, '');
  const outputWavName = `${baseName}.wav`;
  const outputWavPath = path.join(wavDir, outputWavName);
  ...
  const outputMp3Name = `${baseName}.mp3`;
  const outputMp3Path = path.join(mp3Dir, outputMp3Name);
  if (createMp3) {
    const pass2Mp3Args = [
      '-v', 'error',
      '-y',
  ...
  fs.writeFileSync(path.join(reportsDir, `${baseName}.json`), JSON.stringify(result, null, 2));
