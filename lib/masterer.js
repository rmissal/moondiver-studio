...
async function masterSingleFile(inputFile, outputFolder, options = {}) {
  ...
  const cleanTitle = baseName.replace(/^\d+[\s_-]*/, '');
  let outputWavName = `${baseName}${suffix}.wav`;
  let outputWavPath = path.join(wavDir, outputWavName);

  // Single-File Versioning (so we never overwrite)
  if (fs.existsSync(outputWavPath)) {
    let ver = 1;
    while (fs.existsSync(path.join(wavDir, `${baseName}${suffix}_v${ver}.wav`))) ver++;
    outputWavName = `${baseName}${suffix}_v${ver}.wav`;
    outputWavPath = path.join(wavDir, outputWavName);
  }

  const artistName = options.artist || 'SinnTaucher';
  const albumName = options.album || path.basename(path.resolve(outputFolder).replace(/[/\\]mastered_versions$/i, ''));
  ...
  if (createMp3) {
    let mp3Name = `${baseName}${suffix}.mp3`;
    let mp3Path = path.join(mp3Dir, mp3Name);
    if (fs.existsSync(mp3Path)) {
      let ver = 1;
      while (fs.existsSync(path.join(mp3Dir, `${baseName}${suffix}_v${ver}.mp3`))) ver++;
      mp3Name = `${baseName}${suffix}_v${ver}.mp3`;
  ...
  const result = {
    ...
    analysis: {
      measuredLufs: measured.input_i ? parseFloat(measured.input_i) : null,
      measuredTruePeak: measured.input_tp ? parseFloat(measured.input_tp) : null,
      measuredLra: measured.input_lra ? parseFloat(measured.input_lra) : null,
      targetOffset: measured.target_offset ? parseFloat(measured.target_offset) : null,
      normalizationMode: 'Two-Pass Linear (Pumping-Free)'
    },
    settings: pass2.settings
  };
  ...
  result.appleMusicConfidence = calculateAppleMusicScore(appleMetrics);
...
async function masterAudio(targetPath, options = {}) {
  ...
  if (files.length === 0) {
    throw new Error(`Keine passenden Audio-Dateien in '${targetPath}' gefunden.`);
  }

  let outputFolder = options.outputFolder;
  if (!outputFolder) {
    outputFolder = isSingleFile ? directory : path.join(path.resolve(targetPath), 'mastered_versions');
  } else {
  ...
  if (!isSingleFile && fs.existsSync(outputFolder)) {
    ...
    try {
      ...
      console.log(`Verschiedenes existierendes Master-Verzeichnis nach ${path.basename(backupFolder)} verschoben.`);
    } catch (e) {
      console.error(`Konnte existierendes Verzeichnis nicht versionieren: ${e.message}`);
    }
  }
  ...
  return {
    status: 'success',
    message: `${results.length} Datei(en) erfolgreich mit Two-Pass Linear Mastering gemastert.`,
    outputFolder,
