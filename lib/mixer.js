...
async function mixStems(targetFolder, options = {}) {
  ...
  // 1. Vocals (with anti-click fade & bleed gate)
  if (vocalsPath) {
    inputs.push('-i', vocalsPath);
    const v = sm.vocals || { volume: 1.2, highpass: 100, trebleGain: 2.5, trebleFreq: 8000 };
    const vFilters = ['afade=t=in:ss=0:d=0.03:curve=qsin', 'agate=threshold=-55dB:ratio=2:range=-24dB:attack=10:release=100'];
  ...
  // 3. Drums (with anti-click fade & bleed gate)
  if (drumsPath) {
    inputs.push('-i', drumsPath);
    const d = sm.drums || { volume: 1.0, punchFreq: 3500, punchGain: 1.0 };
    const dFilters = ['afade=t=in:ss=0:d=0.03:curve=qsin', 'agate=threshold=-55dB:ratio=2:range=-24dB:attack=10:release=100'];
