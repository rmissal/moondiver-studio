...
function calculateAppleMusicScore(track = {}) {
  ...
  if (tp <= -1.4) {
    truePeakScore = 30;
    truePeakNote = 'Excellent (Zero risk of AAC intersample clipping, <= -1.5 dBTP)';
  } else if (tp <= -1.0) {
    truePeakScore = 28;
    truePeakNote = 'Fully Apple Digital Masters compliant (<= -1.0 dBTP)';
  } else if (tp <= -0.5) {
    truePeakScore = 18;
    truePeakNote = 'Good headroom, minor intersample risk during AAC transcode';
  } else if (tp <= -0.1) {
    truePeakScore = 10;
    truePeakNote = 'Warning: Close to 0 dBTP, risk of intersample clipping';
  } else {
    truePeakScore = 0;
    truePeakNote = 'Critical: Exceeds 0 dBTP True Peak (Clipping & distortion)';
  }

  // 2. Apple Sound Check Loudness Matching (25 Pts)
  let loudnessScore = 0;
  let loudnessNote = '';
  if (lufs >= -16.5 && lufs <= -14.5) {
    loudnessScore = 25;
    loudnessNote = 'Perfect alignment with Apple Sound Check & Spotify (-15 to -16 LUFS)';
  } else if ((lufs >= -18.5 && lufs < -16.5) || (lufs > -14.5 && lufs <= -13.5)) {
    loudnessScore = 22;
    loudnessNote = 'Very Good (Optimally placed in streaming target window)';
  } else if (lufs > -13.5 && lufs <= -11.5) {
    loudnessScore = 14;
    loudnessNote = 'Accepted, but will be attenuated by Apple Sound Check';
  } else {
    loudnessScore = 5;
    loudnessNote = 'Noticeably too loud or quiet for standard streaming normalization';
  }
