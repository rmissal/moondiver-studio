...
export async function masterSingleFile(inputFile, options = {}) {
  ...
  // 1. Analyze Pass 1 & Resolve Effective Preset
  let genreInfo = null;
  const initialAnalysis = await analyzeFile(inputFile);
  let effectivePresetKey = options.preset && options.preset !== 'auto' ? options.preset : 'new_age_ambient';
  if (isAuto) {
  ...
  if (isAuto && initialAnalysis.spectralDNA) {
    if (effectiveOptions.midDeMudGainDb === undefined && initialAnalysis.spectralDNA.autonomousDeMudDb !== 0) {
      effectiveOptions.midDeMudGainDb =
        (PRESETS[effectivePresetKey]?.midDeMudGainDb || 0) + initialAnalysis.spectralDNA.autonomousDeMudDb;
    }
  ...
  for (const match of existingMatches) {
    if (match !== targetWavName) {
      try {
        fs.unlinkSync(path.join(wavDir, match));
      } catch {}
      try {
        fs.unlinkSync(path.join(wavDir, match.replace(/\.wav$/i, '.json')));
      } catch {}
    }
  }
  ...
  const genreStr =
    options.genre ||
    (isAuto && genreInfo ? genreInfo.genreName : PRESETS[effectivePresetKey]?.name || effectivePresetKey);
  ...
  // 3. Measure the REAL finalized master audio KPIs using EBU R128 Pass 1 analysis
  const wavStats = fs.statSync(outputWavPath);
  const finalMasterMetrics = await analyzeFile(outputWavPath);
  const appleScoreObj = finalMasterMetrics.appleMusicConfidence || calculateAppleMusicScore(finalMasterMetrics);
  ...
  const result = {
    ...
    presetName: PRESETS[effectivePresetKey]?.name || effectivePresetKey,
    appliedPreset: effectivePresetKey,
    appliedPresetName: PRESETS[effectivePresetKey]?.name || effectivePresetKey,
    profile: PRESETS[effectivePresetKey]?.name || effectivePresetKey,
    genre: genreStr,
    autoDetectedGenre: isAuto
      ? genreInfo
      : {
          preset: effectivePresetKey,
          name: PRESETS[effectivePresetKey]?.name || effectivePresetKey,
          genreName: PRESETS[effectivePresetKey]?.name || effectivePresetKey,
          detectedPreset: effectivePresetKey
        },

    // BEFORE / AFTER COMPARISON (KPIs - Real Measured Audio Metrics)
    kpiComparison: {
      lufs: {
        beforeMeasured: measured.input_i ? `${measured.input_i} LUFS` : 'N/A',
        afterMeasured: `${finalMasterMetrics.integratedLoudnessLufs} LUFS`,
        afterTarget: `${pass2.settings.targetLufs} LUFS`,
        before_measured: measured.input_i ? `${measured.input_i} LUFS` : 'N/A',
        after_measured: `${finalMasterMetrics.integratedLoudnessLufs} LUFS`,
        after_target: `${pass2.settings.targetLufs} LUFS`
      },
      truePeak: {
        beforeMeasured: measured.input_tp ? `${measured.input_tp} dBTP` : 'N/A',
        afterMeasured: `${finalMasterMetrics.truePeakDbtp} dBTP`,
        afterTarget: `${pass2.settings.truePeak} dBTP`,
        before_measured: measured.input_tp ? `${measured.input_tp} dBTP` : 'N/A',
        after_measured: `${finalMasterMetrics.truePeakDbtp} dBTP`,
        after_target: `${pass2.settings.truePeak} dBTP`
      },
      loudnessRange: {
        beforeMeasured: measured.input_lra ? `${measured.input_lra} LU` : 'N/A',
        afterMeasured: `${finalMasterMetrics.loudnessRangeLra} LU`,
        afterTarget: `${pass2.settings.lra} LU`,
        before_measured: measured.input_lra ? `${measured.input_lra} LU` : 'N/A',
        after_measured: `${finalMasterMetrics.loudnessRangeLra} LU`,
    ...
    optimizations: [
      ...
      ...(isAuto && initialAnalysis.spectralDNA
        ? [
            ...
            ...(initialAnalysis.spectralDNA.needsBassMonomaker
              ? ['Autonomous Action: Applied M/S Bass Monomaker']
              : []),
            ...(initialAnalysis.spectralDNA.autonomousDeMudDb !== 0
              ? [`Autonomous Action: Adaptive De-Mudding (${initialAnalysis.spectralDNA.autonomousDeMudDb}dB)`]
              : []),
            ...(initialAnalysis.spectralDNA.needsAir
              ? ['Autonomous Action: Psychoacoustic Harmonic Exciter (Air Sheen) applied']
              : [])
          ]
        : []),
