  ...
  return new Promise((resolve, reject) => {
    ...
    p.on('close', (code) => {
      if (code === 0) {
        ...
        reject(new Error(`Befehl '${cmd}' schlug fehl mit Code ${code}:\n${stderr}`));
      }
    });
...
async function embedStudioMetadata(filePath, meta = {}) {
  ...
  const artist = meta.artist || 'SinnTaucher';
  ...
  const tempOut = path.join(os.tmpdir(), `studio_meta_${Date.now()}_${Math.random().toString(36).slice(2)}_${path.basename(filePath)}`);

  const args = [
    '-i', filePath,
    '-c', 'copy',
    '-map_metadata', '-1', // Entfernt alle Suno-Tags und interne IDs
