...
try {
  ...
  try {
    ...
    if (isWin) {
      try {
        ...
        throw new Error('Python was not found. Please install Python 3.', { cause: err });
      }
    } else {
      throw new Error('python3 was not found. Please install Python 3.', { cause: e });
    }
  }
  ...
  if (isWin) {
    ...
    execSync(
      `${pyCmd} -m pip install --pre torch torchvision torchaudio --index-url https://download.pytorch.org/whl/nightly/cu128 --upgrade`,
      { stdio: 'inherit' }
    );
  } else {
  ...
  if (isWin) {
    ...
    if (sitePackages) {
      execSync(
        `powershell -Command "Get-ChildItem -Path '${sitePackages}' -Recurse | Unblock-File -ErrorAction SilentlyContinue"`,
        { stdio: 'inherit' }
      );
